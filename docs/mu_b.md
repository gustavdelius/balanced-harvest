...
```
mu_b,i(x,t) = mu_b,i^(0) exp(-xi (x - x0,i)) g_i(x,t)/g_i(x0,i,t)      (A.7)
```
...
| option | form |
|---|---|
| `mu_b_form = "product"` | `mu_b0 (w/w0)^-xi * [g(w,t)/g(w0,t)]` - as printed |
| `mu_b_form = "ratio"` | `mu_b0 (w/w0)^-xi / [g(w,t)/g(w0,t)]` - as described |

## Why this implementation defaults to "ratio"

The two are not a minor detail: `g(w,t)/g(w0,t)` falls by about an order of
...
| `w_max` | form | mortality at `w_max/2` | total biomass | biomass above 400 g |
|---|---|---|---|---|
...
2. **Total yield.** The paper reports ~0.25 g/m2/yr at `F = 0.1 /yr` over
   `w > 400 g`, which needs `sum_i B_i(>400 g)` of about 2.5 g/m2. Under
...
3. **Which species are rare.** In Figs 2c and 3a the *small*-`w_max` species
   (1, 2, 3, 4) are the common ones and the rare ones sit at larger `w_max`.
   Under "product", adult mortality is effectively zero, biomass piles up at
   `w_max` where growth stops, and the largest species always dominates - small
   species cannot invade at all. Under "ratio", mortality rises with `w_max`
