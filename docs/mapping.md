...
| Eq. (A.8) allocation `1 - eps_i` | `psi = maturity * repro_prop`; `w_mat25 = w_mat/3^(1/rho_m)` gives mizer's maturity exponent `U = rho_m = 15`; `m - n = rho_inf = 0.2` gives `repro_prop = (w/w_max)^0.2` |
...
| Appendix C `theta` | `interaction` matrix (0.5 on the diagonal, 0.2 off) and `interaction_resource = 1` |
...
Eq. (A.1f) is written in log mass as `(1/2) d/dx(e^-x d/dx[eps_i G_i u_i])`,
with `G_i` (Eq. A.10) the second moment of prey mass. mizer works in `w` and
adds `(1/2) d2/dw2(D_i N_i)` with `D_i` in g2/yr. The two are the same term.
Writing `B = eps_i G_i u_i` and using `d/dx = w d/dw` and `e^-x = 1/w`
(with `w0 = 1 g`),

```
(1/2) d/dx( (1/w) d/dx B ) = (1/2) w d/dw( (1/w) * w dB/dw ) = (1/2) w d2B/dw2
```

and the whole of Eq. (A.1) is an equation for `u = w phi`, so dividing through
by `w` leaves `(1/2) d2B/dw2`. Since `G_i = D_i/w` and `u_i = w phi_i`,
`B = eps_i D_i phi_i`, which is mizer's term with the `eps_i` factor that
`mizerDiffusion()` omits and `lpDiffusion()` restores. The same division by `w`
turns Eq. (A.1a) into mizer's advection term, so the correspondence is
term-by-term, not just in aggregate.
...
1. **Eq. (A.3) as printed evaluates the predator's search area at the prey's
   body mass** (`A_j (w0 e^x)^alpha_j`, where `x` indexes the prey). Eq. (A.2)
...
3. **Table 1 contradicts the text on `x_a` and `x_I`.** The table glosses
   `w0 e^-x_a` as 0.001 g, which with `a(x) = a0 (w0 e^(x - x_a))^(1 - lambda)`
   would put the carrying capacity at 2e9 m^-2 at 1 mg rather than the stated
   2000 m^-2. The text's `x_a = log(0.001)` gives `a(w) = a0 (w/0.001)^(1-lambda)`,
   which does put it at 2000 m^-2 at 1 mg, and integrates to a total primary
   production of ~4.4e3 g m^-2 yr^-1 against the paper's "region of 4000". The
   text is therefore taken to be correct.
...
4. **The invading spectrum.** Appendix B says only "started at a low density on
   ...
   `phi(w_egg) = 2 m^-2 g^-1`, on a power law `u ~ w^-(lambda-1)` matching the
   plankton spectrum. Only the invasion *outcome* depends on this, and only
...
2. **The particular ecosystems differ.** The paper does not report the random
   seed or the realised `w_max` and `mu_egg` of its 15 species, so the assembly
