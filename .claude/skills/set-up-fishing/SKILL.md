...
description: >-
  ...
  with the initial_effort replacement accessor. Note that catchability sets the
  units in which effort is measured. Matching modelled yields to observed ones
...
and `gear_params(params) <- ...`. Core columns:
...
gear–species combination. Both the `species` and `gear` columns must be present,
although `NA` gear entries are replaced by their species name. Mizer fills the
other core columns with the defaults shown in the table above. You must supply
the parameter columns of whatever non-default `sel_func` you choose. Here two
gears fish cod, each with its own length-based selectivity:
